...
import com.clipnest.data.repository.localizedCaptureSourceLabel
import com.clipnest.data.repository.ClipboardRepository
import com.clipnest.domain.ExportFormatter
import com.clipnest.domain.ExportLabels
...
class VaultViewModel(
    ...
    fun togglePinSelected() {
        ...
        viewModelScope.launch {
            ...
            _selectedIds.value = emptySet()
            emitPluralToast(
                if (newPinned) com.clipnest.R.plurals.pinned_items else com.clipnest.R.plurals.unpinned_items,
                selected.size
            )
        }
    }
    ...
    fun confirmDeleteSelected() {
        ...
        viewModelScope.launch {
            ...
            _selectedIds.value = emptySet()
            emitPluralToast(com.clipnest.R.plurals.deleted_items, selected.size)
        }
    }
        ...
        viewModelScope.launch {
            ...
            _eventFlow.emit(
                VaultEvent.ShareText(
                    text = shareText,
                    chooserTitle = localizedContext().getString(com.clipnest.R.string.share_selected_cards)
                )
            )
        }
    }
    ...
    fun copySingleCard(context: Context, id: Long) {
        viewModelScope.launch {
            ...
            val clip = ClipData.newPlainText(
                localizedContext().getString(com.clipnest.R.string.clipboard_card_label),
                card.content
            )
            clipboard.setPrimaryClip(clip)
            emitToast(com.clipnest.R.string.copied_to_clipboard)
        }
    }
            ...
            _eventFlow.emit(
                VaultEvent.ShowToast(
                    localizedContext().getString(
                        if (saved.isNotEmpty()) com.clipnest.R.string.clipboard_saved else com.clipnest.R.string.clipboard_empty
                    )
                )
            )
        }
    ...
    fun saveInAppCapture(text: String, isSensitive: Boolean) {
        viewModelScope.launch {
            ...
            _showInAppCaptureSheet.value = false
            if (card != null) {
                emitToast(com.clipnest.R.string.clipboard_saved)
            }
        }
    }
        ...
        val payloads = when {
            clipText != null && shared != null && clipText != shared -> listOf(
                CapturePayload(
                    ...
                    contentType = com.clipnest.data.model.ContentType.COMBINED
                )
            )
    ...
    private fun localizedExportLabels(): ExportLabels {
        ...
        return ExportLabels(
            title = context.getString(com.clipnest.R.string.export_title),
            noItems = context.getString(com.clipnest.R.string.export_no_items),
            exportedOn = context.getString(com.clipnest.R.string.exported_on),
            date = context.getString(com.clipnest.R.string.export_date),
            source = context.getString(com.clipnest.R.string.export_source).substringBefore(":"),
            item = context.getString(com.clipnest.R.string.export_item),
            captured = context.getString(com.clipnest.R.string.export_captured),
            pinned = context.getString(com.clipnest.R.string.export_pinned),
            urlType = context.getString(com.clipnest.R.string.export_url_type),
            combinedType = context.getString(com.clipnest.R.string.export_combined_type),
