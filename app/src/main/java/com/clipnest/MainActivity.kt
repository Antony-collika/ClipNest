...
class MainActivity : ComponentActivity() {
                    ...
                    backupPasswordRequest.value?.let { action ->
                        BackupPasswordDialog(
                            ...
                            onConfirm = { password ->
                                ...
                                if (action == BackupPasswordAction.EXPORT) {
    ...
    private fun requestBackupFile() {
        backupPasswordRequest.value = BackupPasswordAction.EXPORT
    }

    private fun requestRestoreFile() {
        backupPasswordRequest.value = BackupPasswordAction.IMPORT
    }
    ...
    private fun launchBackupFilePicker() {
        ...
        backupFileLauncher.launch("ClipNest_Backup_$date.clipnest.json")
    }
    ...
    private fun writeVaultBackup(uri: Uri, password: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            val result = runCatching {
                val json = VaultBackupCodec.encodeEncrypted(repository.getAllCards(), password)
            ...
            val message = applicationContext.withAppLanguage(language).getString(
                when {
                    result.isSuccess &&
                        ...
                        result.getOrNull()?.skippedDuplicates == 0 -> com.clipnest.R.string.restore_empty
                    ...
                    result.exceptionOrNull() is IllegalArgumentException ||
                        ...
                        result.exceptionOrNull() is com.squareup.moshi.JsonEncodingException -> com.clipnest.R.string.invalid_backup_file
    ...
    private fun shareTextExternally(text: String, chooserTitle: String) {
        ...
        runCatching {
            ...
            Toast.makeText(
                this,
                getString(com.clipnest.R.string.share_unavailable),
                Toast.LENGTH_SHORT
            ).show()
                }
    }
    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (action == Intent.ACTION_SEND_MULTIPLE && candidates.isEmpty()) {
            Log.w("ClipNest.OpenWith", "Multiple-send intent did not contain document URIs")
            return
        }
        ...
        if (uri == null) {
            Log.w("ClipNest.OpenWith", "Open intent did not contain a supported document URI")
            return
        }
        ...
        if (uri.scheme.equals(ContentResolver.SCHEME_CONTENT, ignoreCase = true) &&
            ...
            Log.w("ClipNest.OpenWith", "Content URI has no explicit read grant: $uri")
        }
        if (grantedFlags != 0 &&
            ...
            runCatching { contentResolver.takePersistableUriPermission(uri, grantedFlags) }
                .onFailure { error ->
                    Log.d("ClipNest.OpenWith", "Persistable permission unavailable for $uri", error)
                }
        }
...
fun MainAppContent(
    ...
    Scaffold(
        topBar = {
            MainTopBar(
                    title = when {
                        isSettings -> stringResource(com.clipnest.R.string.settings)
                        selectedTab == 0 -> stringResource(com.clipnest.R.string.vault)
                        else -> if (editorUiState.externalDocumentUri != null) editorUiState.documentName else stringResource(com.clipnest.R.string.editor)
                    },
                        ...
                        Row(
                            ...
                            if (isEditor) {
                                ...
                                IconButton(
                                    ...
                                    Icon(Icons.Default.KeyboardArrowUp, contentDescription = stringResource(com.clipnest.R.string.previous_match))
                                }
                                IconButton(
                                    onClick = onNextSearchMatch,
                                    modifier = Modifier.size(36.dp).testTag("editor_search_next")
                                ) {
                                    Icon(Icons.Default.KeyboardArrowDown, contentDescription = stringResource(com.clipnest.R.string.next_match))
                                }
                            }
                        }
                    }
                    ...
                    isVault || isEditor -> {
                        Row(
                            ...
                            MainTabSlot(
                                ...
                                Text(
                                    stringResource(com.clipnest.R.string.vault),
                ...
                if (!isSettings) {
                    if (isSearchOpen) {
                        ...
                        IconButton(
                            ...
                            Icon(Icons.Default.Search, contentDescription = stringResource(com.clipnest.R.string.search), modifier = Modifier.size(22.dp))
                        }
                    }
                    ...
                    Box {
                        IconButton(
                            ...
                            Icon(Icons.Default.MoreVert, contentDescription = stringResource(com.clipnest.R.string.more_options), modifier = Modifier.size(22.dp))
                        }
                        if (overflowExpanded) {
                            DropdownMenu(
                    ...
                    if (isVault) {
                        DropdownMenuItem(
                            text = { Text(if (allSelected) stringResource(com.clipnest.R.string.clear_selection) else stringResource(com.clipnest.R.string.select_all)) },
                            onClick = {
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(com.clipnest.R.string.share)) },
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(com.clipnest.R.string.show_pinned_first)) },
                            trailingIcon = {
                                if (showPinnedFirst) {
                                    Icon(Icons.Default.Check, contentDescription = stringResource(com.clipnest.R.string.active))
                                }
                            },
                            onClick = {
                                overflowExpanded = false
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(com.clipnest.R.string.open_file)) },
                            onClick = {
                                overflowExpanded = false
                                onOpenFile()
                            },
                        ...
                        DropdownMenuItem(
                            text = { Text(stringResource(com.clipnest.R.string.return_to_editor)) },
